...
def _depth_basename(rgb_basename):
    stem, _ = osp.splitext(rgb_basename)
    return f"{stem}.png"


def _resize_depth_to_match_rgb(depthmap, rgb_image):
    target_h, target_w = rgb_image.shape[:2]
    if tuple(depthmap.shape[:2]) == (target_h, target_w):
        return depthmap
    return cv2.resize(depthmap, (target_w, target_h), interpolation=cv2.INTER_NEAREST)
...
class ARKitScenes_Multiview(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        for view_idx in image_indices:
            ...
            depth_basename = _depth_basename(basename)

            # Load RGB image
            rgb_image = imread_cv2(osp.join(scene_dir, 'vga_wide', basename))
            # Load depthmap
            depthmap = imread_cv2(osp.join(scene_dir, 'lowres_depth', depth_basename), cv2.IMREAD_UNCHANGED)
            depthmap = depthmap.astype(np.float32) / 1000
            depthmap[~np.isfinite(depthmap)] = 0  # invalid
            depthmap = _resize_depth_to_match_rgb(depthmap, rgb_image)
