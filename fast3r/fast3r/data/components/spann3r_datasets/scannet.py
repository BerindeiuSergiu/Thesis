...
class Scannet(BaseManyViewDataset):
    def __init__(self, num_seq=100, num_frames=5, 
                 ...
                 kf_every=1, max_scenes=None,
                 frame_stride=1, blur_filter=False,
                 blur_threshold=40.0,
                 scene_allowlist_path=None,
                 *args, ROOT, **kwargs):
        self.ROOT = ROOT
        ...
        self.scene_allowlist_path = scene_allowlist_path
    ...
    def load_all_scenes(self, base_dir):
        ...
        if self.test_id is None:
            ...
            if self.scene_allowlist_path is not None:
                with open(self.scene_allowlist_path, "r", encoding="utf-8") as handle:
                    allowlist = {line.strip() for line in handle if line.strip()}
                self.scene_list = [scene for scene in self.scene_list if scene in allowlist]
            if self.max_scenes is not None:
                self.scene_list = self.scene_list[: int(self.max_scenes)]
    ...
    def _get_views(self, idx, resolution, rng, attempts=0): 
        ...
        img_idxs_ = img_idxs_[:: self.frame_stride]
        ...
        while len(imgs_idxs) > 0:
            ...
            if self.blur_filter and self._blur_score(rgb_image) < self.blur_threshold:
                if self.full_video:
                    continue
                else:
                    continue
        ...
        if len(views) < self.num_frames:
            if attempts >= 5:
                new_idx = rng.integers(0, self.__len__() - 1)
                return self._get_views(new_idx, resolution, rng, attempts=0)
            return self._get_views(idx, resolution, rng, attempts + 1)
