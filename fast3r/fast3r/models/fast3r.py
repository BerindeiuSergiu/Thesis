...
def _safe_distributed_rank() -> int:
    """Return distributed rank when available, otherwise 0."""
    dist = getattr(torch, "distributed", None)
    if dist is None:
        return 0
    is_initialized = getattr(dist, "is_initialized", None)
    get_rank = getattr(dist, "get_rank", None)
    if not callable(is_initialized) or not callable(get_rank):
        return 0
    try:
        return int(get_rank()) if is_initialized() else 0
    except Exception:
        return 0
    ...
    def forward(self, views, profiling=False):
        ...
        if encode_images_end_time - encode_images_start_time > 20:
            ...
            for view_idx, view in enumerate(views):
                print(f"view_idx: {view_idx}, true_shape: {view['true_shape']}")
                # Skip view_name(view) as it requires 'dataset' key which we don't have
...
class Fast3RDecoder(nn.Module):
    ...
    def _generate_per_rank_generator(self):
        ...
        world_rank = _safe_distributed_rank()
