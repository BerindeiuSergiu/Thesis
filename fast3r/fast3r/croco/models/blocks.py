...
class Attention(nn.Module):
    ...
    def forward(self, x, xpos):
        ...
        if self.attn_implementation == "pytorch_naive":
            ...
            # Try Flash Attention, fall back to naive if not available
            try:
                ...
                # Fallback to naive PyTorch implementation
                assert self.attn_mask is None, "attn_mask not supported for pytorch_naive fallback"
                assert self.is_causal is False, "is_causal not supported for pytorch_naive fallback"
                dtype = k.dtype
                with torch.autocast("cuda", dtype=torch.bfloat16):
                    x = (q @ k.transpose(-2, -1)) * scale
                    x = x.softmax(dim=-1)
